...
TODO: make a private git repo, and put the binary in it
  ...
  - only need one version.. so i don't need to login to itch.io again to download it..
    
  - set-up as git module to the game repo

NVM: setup company-dragonruby for emacs
...
*it might just be easier to read the framework/run-time..*
  - *at the minimum, it's very helpful..*
  - "the up to date contrib repo contents are actually in docs/oss"
  - the old(?) repo
    - https://github.com/DragonRuby/dragonruby-game-toolkit-contrib

* docs
single file static local docs
  - [[file:~/my-stuff/repos/dragonruby/dragonruby-linux-amd64/docs/static/docs.html]]
  - [[file:~/my-stuff/repos/dragonruby/dragonruby-linux-amd64/docs/static/docs.txt]]
    - *actually a .org file!!*
...
either:
1. use the whole dragonruby folder as a repo (and keep a copy of the whole folder for every game)
   - make sure you get the right .gitignore from the docs!
     - (do not upload the core dragonruby exe's!)
     - it should only push the /mygame folder
     
2. keep a single folder of dragonruby, *make/copy a repo in /mygame and directly edit the repo there* (and swap out /mygame repo for every game)
  - this .gitignore is much simpler
  - keep a copy of the contents of 'VERSION.txt' appended to the top of main.rb
    - probably don't need to do this.. amir/dragonruby is quite smart..
      
regardless of which way:    
*create a private repo with a copy of dragonruby un-zipped, with .gitignore only '/tmp/' and '/logs/'*
  - *this is just to keep the right version for future-proofing..*
    - *TODO: should download the arm64 version too!*
      - for mobile devices
  - ~100mb each
    - mostly images for its many included samples..
    - (dragonruby run-time is only 4mb tho..)

from the docs for public repos:
...
if linux, can add to path in .bashrc, to make it easier to run
#+begin_src sh
    export PATH="~/my-stuff/repos/dragonruby/dragonruby-linux-amd64/dragonruby:$PATH"
#+end_src
...
** text editor setup
*** NO emacs company-mode

WARNING: old prototype / not "production-ready"
...
Code
    ...
    ;; NOTE: cape-company-to-capf in cape.el converts company stuff to emacs native capf (completion-at-point-functions)
...
NOTE: *though much simpler and less resource-intensive than a lsp, ALL of the completions are jumbled up.. :/. there is no object/module hierarchy to organize completions*
...
TODO: merge with ruby.org lsp notes
...
# gem install yard # solargraph includes this
#+end_src

get the dr yard docs repo
  - https://github.com/owenbutler/dragonruby-yard-doc
    - check the readme, in case anything changed
  - NOTE: i added this as a sub-module to the dragonruby repo
  - TODO: can you just use docs/oss instead?
    
add the following to solargraph.yml:
#+begin_src yaml
include:
- "mygame/app/*.rb"
...
in mygame folder, run `solargraph config`
  - generates solargaph.yml
set up the relative directory to the yard docs repo and app folder

NOTE: i don't think you need to actually generate docs (via 'yard doc .' / 'yardoc'). it reads the files directly, generating it's own stuff / caching data to a share location ('~/.cache/solargraph'). "It generates this information automatically at runtime".

NOTE: if emacs, the lsp(-mode) may ask within a pop-up for the root of the project (in case you open a buffer that isn't part of the project's root). You must set that in order to auto-start the lsp.

i recall it at least categorizing completions, with the current class's methods on top, then super/inherited objects below

TODO: but now, with the prism parser, surely there's a better one..??
  - https://github.com/rajnaveen344/ruby-fast-lsp
    - could try it out..
...
def tick args
  args.
    ...
    # i think it just doesn't work with the attr_gtk module/macro thing..
    # or maybe it was the type hint that made it work..?
end

$game.
  # shows everything in the Game class, which should be everything i write..

han
  # typing 3 characters triggers the lsp
  # i think the minimum is one character..
...
**** optimal ways / troubleshoot performance

see "troubleshoot performance" in the tutorial
...
UPDATE: NOTE: found a docs.txt that was a .org file!
  - thank goodness..! that generated html file sucked!
  - not sure if i need these notes anymore..

my notes while going over the tutorial in the docs
  - it's mostly copy-pasta from the single page docs, which just wasn't very organized..

TODO: can extract the important bits out later, once i'm more comfy with the engine and language
  - or maybe can keep as a refresher..
